...
import { dyslexiaDyscalculiaCategories, dyslexiaDyscalculiaDidYouKnow } from "@/data/conditions/dyslexia-dyscalculia";
import { getPlanetTheme } from "@/lib/planetThemes";

interface DyslexiaDyscalculiaPageProps {
  onBack?: () => void;
}

export function DyslexiaDyscalculiaPage({ onBack }: DyslexiaDyscalculiaPageProps = {}) {
  const theme = getPlanetTheme('dyslexia-dyscalculia');
  ...
  return (
    <div className="relative min-h-screen">
      ...
      {/* Content */}
      <div className="max-w-7xl mx-auto px-4 py-6 planet-dyslexia-dyscalculia relative" style={{ zIndex: 10 }}>
        {/* Header section */}
        ...
        <h1 className="text-4xl md:text-5xl font-bold mb-4 text-slate-100">
          Welcome to the Dyslexia & Dyscalculia Planet 📖
        </h1>

        <p className="text-lg md:text-xl text-slate-300 max-w-2xl mx-auto mb-4">
          Reading tools, maths support, assistive technology
        </p>

        <p className="text-base text-slate-400 max-w-xl mx-auto italic">
          Celebrating your unique brain and finding what works for you
        </p>
        ...
        <div className="relative mb-8 sm:mb-10 md:mb-12 lg:mb-16 animate-slide-up" style={{ animationDelay: '200ms', zIndex: 10 }}>
          ...
          <FactCarousel facts={dyslexiaDyscalculiaDidYouKnow} colour="var(--planet-primary)" />
        </div>
          ...
          >
            ...
            >
              <div className="space-y-3">
                {dyslexiaDyscalculiaCategories.map((category) => (
