...
export function DyscalculiaHowItShowsUp() {
  ...
  const goToNext = () => {
    if (currentIndex < dyscalculiaPlanetMobile.howItShowsUp.length - 1) {
      setCurrentIndex(currentIndex + 1);
    }
  };
  ...
  const currentSection = dyscalculiaPlanetMobile.howItShowsUp[currentIndex];
  ...
  return (
    <PlanetLayout
      ...
      primaryColor="#FFD966"
      accentColor="#F5C542"
      nextRoute="/planets/dyscalculia/strengths"
      prevRoute="/planets/dyscalculia/understanding"
      ...
      <div className="flex flex-col items-center justify-center min-h-screen py-12 px-6" style={{ maxWidth: "600px", margin: "0 auto" }}>
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-slate-400 text-base" style={{ lineHeight: "1.6", opacity: 0.7 }}>
            How dyscalculia might affect your life
          </p>
        </div>

        {/* Progress indicator */}
        <div className="flex gap-2 mb-8">
          {dyscalculiaPlanetMobile.howItShowsUp.map((section, idx) => (
            <button
              key={section.id}
        ...
        <div className="flex items-center justify-between w-full mt-12" style={{ maxWidth: "400px" }}>
          ...
          <div className="text-center">
            <p className="text-slate-400 text-sm" style={{ opacity: 0.7 }}>
              {currentIndex + 1} of {dyscalculiaPlanetMobile.howItShowsUp.length}
            </p>
          </div>

          <button
            onClick={goToNext}
            disabled={currentIndex === dyscalculiaPlanetMobile.howItShowsUp.length - 1}
            className="transition-all duration-200"
            style={{
              opacity: currentIndex === dyscalculiaPlanetMobile.howItShowsUp.length - 1 ? 0.3 : 0.6,
              cursor: currentIndex === dyscalculiaPlanetMobile.howItShowsUp.length - 1 ? "not-allowed" : "pointer",
              background: "none",
              border: "none",
              fontSize: "2rem",
              color: currentSection.accentColor,
              filter: currentIndex === dyscalculiaPlanetMobile.howItShowsUp.length - 1 ? "none" : `drop-shadow(0 0 4px ${currentSection.accentColor}40)`
            }}
