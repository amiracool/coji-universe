...
export function IdeasQuestionsStrengths() {
  const router = useRouter();
  const [currentIndex, setCurrentIndex] = useState(0);

  const strengths = ideasQuestionsPlanetMobile.strengthsAndSensitivities;
  const superpowers = ideasQuestionsPlanetMobile.superpowersFacts;
  const [currentSuperpowerIndex, setCurrentSuperpowerIndex] = useState(0);

  const handleNext = () => {
    router.push('/planets/ideas-questions/planning');
  };

  const handlePrev = () => {
    router.push('/planets/ideas-questions/how-it-shows-up');
  };
  ...
  return (
    <PlanetLayout
      currentStep={5}
      totalSteps={6}
      nextRoute="/planets/ideas-questions/planning"
      prevRoute="/planets/ideas-questions/how-it-shows-up"
      ...
      <div className="space-y-10 py-8">
        ...
        <div className="text-center space-y-3">
          ...
          <p className="text-slate-400 text-sm max-w-md mx-auto">
            Your curiosity and questions are gifts
          </p>
        </div>
        ...
        <div className="flex justify-center gap-2">
          {strengths.map((_, index) => (
            <button
              ...
              style={{
                background: index === currentIndex ? "#A78BFA" : "rgba(167, 139, 250, 0.3)",
          ...
          <div className="flex justify-center gap-2 pt-4">
            {superpowers.map((_, index) => (
              <button
                ...
                className="w-2 h-2 rounded-full transition-all duration-200"
                style={{
                  background: index === currentSuperpowerIndex ? "#C4B5FD" : "rgba(196, 181, 253, 0.3)",
