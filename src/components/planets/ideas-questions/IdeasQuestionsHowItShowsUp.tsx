...
export function IdeasQuestionsHowItShowsUp() {
  ...
  const handleNext = () => {
    router.push('/planets/ideas-questions/strengths');
  };
  ...
  const goToNext = () => {
    if (currentIndex < ideasquestionsPlanetMobile.howItShowsUp.length - 1) {
      setCurrentIndex(currentIndex + 1);
    }
  };
  ...
  const currentSection = ideasquestionsPlanetMobile.howItShowsUp[currentIndex];
  ...
  return (
    <PlanetLayout
      ...
      primaryColor="#A78BFA"
      accentColor="#C4B5FD"
      nextRoute="/planets/ideas-questions/strengths"
      prevRoute="/planets/ideas-questions/understanding"
      ...
      <div className="flex flex-col items-center justify-center min-h-screen py-12 px-6" style={{ maxWidth: "600px", margin: "0 auto" }} onClick={handlePageClick}>
        ...
        <div className="text-center mb-12">
          ...
          <h2 className="text-3xl font-light text-slate-100 mb-4" style={{ letterSpacing: "0.02em" }}>
            How IdeasQuestions Shows Up
          </h2>
          <p className="text-slate-400 text-base" style={{ lineHeight: "1.6", opacity: 0.7 }}>
            How ideas questions might affect your life
          </p>
        </div>

        {/* Progress indicator */}
        <div className="flex gap-2 mb-8">
          {ideasquestionsPlanetMobile.howItShowsUp.map((section, idx) => (
            <button
              key={section.id}
        ...
        <div className="flex items-center justify-between w-full mt-12" style={{ maxWidth: "400px" }}>
          ...
          <div className="text-center">
            <p className="text-slate-400 text-sm" style={{ opacity: 0.7 }}>
              {currentIndex + 1} of {ideasquestionsPlanetMobile.howItShowsUp.length}
            </p>
          </div>

          <button
            onClick={goToNext}
            disabled={currentIndex === ideasquestionsPlanetMobile.howItShowsUp.length - 1}
            className="transition-all duration-200"
            style={{
              opacity: currentIndex === ideasquestionsPlanetMobile.howItShowsUp.length - 1 ? 0.3 : 0.6,
              cursor: currentIndex === ideasquestionsPlanetMobile.howItShowsUp.length - 1 ? "not-allowed" : "pointer",
              background: "none",
              border: "none",
              fontSize: "2rem",
              color: currentSection.accentColor,
              filter: currentIndex === ideasquestionsPlanetMobile.howItShowsUp.length - 1 ? "none" : `drop-shadow(0 0 4px ${currentSection.accentColor}40)`
            }}
