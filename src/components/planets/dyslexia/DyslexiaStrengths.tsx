...
export function DyslexiaStrengths() {
  const router = useRouter();
  const [currentIndex, setCurrentIndex] = useState(0);

  const strengths = dyslexiaPlanetMobile.strengthsAndSensitivities;
  const superpowers = dyslexiaPlanetMobile.superpowersFacts;
  const [currentSuperpowerIndex, setCurrentSuperpowerIndex] = useState(0);

  const handleNext = () => {
    router.push('/planets/dyslexia/planning');
  };
  ...
  return (
    <PlanetLayout
      currentStep={5}
      totalSteps={6}
      nextRoute="/planets/dyslexia/planning"
      prevRoute="/planets/dyslexia/how-it-shows-up"
      ...
      <div className="space-y-10 py-8">
        ...
        <div className="text-center space-y-3">
          <h2 className="text-3xl font-bold text-slate-100 mb-3">
            Strengths & Sensitivities of Dyslexia
          </h2>
        ...
        <div className="flex justify-center gap-2">
          {strengths.map((_, index) => (
            <button
              ...
              style={{
                background: index === currentIndex ? "#FF6B9D" : "rgba(255, 107, 157, 0.3)",
        ...
        <div className="pt-8">
          ...
          <div className="relative max-w-lg mx-auto">
            <div
              ...
              style={{
                background: "linear-gradient(135deg, rgba(255, 143, 171, 0.2) 0%, rgba(255, 107, 157, 0.15) 100%)",
          ...
          <div className="flex justify-center gap-2 pt-4">
            {superpowers.map((_, index) => (
              <button
                ...
                className="w-2 h-2 rounded-full transition-all duration-200"
                style={{
                  background: index === currentSuperpowerIndex ? "#FF8FAB" : "rgba(255, 143, 171, 0.3)",
