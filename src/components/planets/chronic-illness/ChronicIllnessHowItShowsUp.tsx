...
export function ChronicIllnessHowItShowsUp() {
  ...
  const handleNext = () => {
    router.push('/planets/chronic-illness/strengths');
  };
  ...
  const goToNext = () => {
    if (currentIndex < chronicillnessPlanetMobile.howItShowsUp.length - 1) {
      setCurrentIndex(currentIndex + 1);
    }
  };
  ...
  const currentSection = chronicillnessPlanetMobile.howItShowsUp[currentIndex];
  ...
  return (
    <PlanetLayout
      ...
      primaryColor="#6EBD8E"
      accentColor="#8ECDA7"
      nextRoute="/planets/chronic-illness/strengths"
      prevRoute="/planets/chronic-illness/understanding"
      ...
      <div className="flex flex-col items-center justify-center min-h-screen py-12 px-6" style={{ maxWidth: "600px", margin: "0 auto" }}>
        ...
        <div className="text-center mb-12">
          ...
          <h2 className="text-3xl font-light text-slate-100 mb-4" style={{ letterSpacing: "0.02em" }}>
            How ChronicIllness Shows Up
          </h2>
          <p className="text-slate-400 text-base" style={{ lineHeight: "1.6", opacity: 0.7 }}>
            How chronic illness might affect your life
          </p>
        </div>

        {/* Progress indicator */}
        <div className="flex gap-2 mb-8">
          {chronicillnessPlanetMobile.howItShowsUp.map((section, idx) => (
            <button
              key={section.id}
        ...
        <div className="flex items-center justify-between w-full mt-12" style={{ maxWidth: "400px" }}>
          ...
          <div className="text-center">
            <p className="text-slate-400 text-sm" style={{ opacity: 0.7 }}>
              {currentIndex + 1} of {chronicillnessPlanetMobile.howItShowsUp.length}
            </p>
          </div>

          <button
            onClick={goToNext}
            disabled={currentIndex === chronicillnessPlanetMobile.howItShowsUp.length - 1}
            className="transition-all duration-200"
            style={{
              opacity: currentIndex === chronicillnessPlanetMobile.howItShowsUp.length - 1 ? 0.3 : 0.6,
              cursor: currentIndex === chronicillnessPlanetMobile.howItShowsUp.length - 1 ? "not-allowed" : "pointer",
              background: "none",
              border: "none",
              fontSize: "2rem",
              color: currentSection.accentColor,
              filter: currentIndex === chronicillnessPlanetMobile.howItShowsUp.length - 1 ? "none" : `drop-shadow(0 0 4px ${currentSection.accentColor}40)`
            }}
