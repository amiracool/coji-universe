...
export function NDWorkHowItShowsUp() {
  const router = useRouter();
  const [expandedId, setExpandedId] = useState<string | null>(null);
  const sections = ndworkPlanetMobile.howItShowsUp;

  const handleNext = () => router.push('/planets/nd-work/strengths');
  const handlePrev = () => router.push('/planets/nd-work/understanding');
  const toggleExpand = (id: string) => setExpandedId(expandedId === id ? null : id);

  return (
    <PlanetLayout currentStep={4} totalSteps={6} nextRoute="/planets/nd-work/strengths" prevRoute="/planets/nd-work/understanding" onSwipeLeft={handleNext} onSwipeRight={handlePrev} primaryColor="#7CD2A1" accentColor="#52B788">
      <div className="space-y-6 py-8">
        <div className="text-center space-y-3 mb-8">
          <h2 className="text-3xl font-bold text-slate-100">How ND & Work Shows Up</h2>
          <p className="text-slate-400 text-base" style={{ lineHeight: "1.6", opacity: 0.7 }}>How ND & Work might affect your life</p>
        </div>
