...
export function DepressionPlanning() {
  ...
  const nextCategory = () => {
    if (focusedCategoryIndex < depressionPlanetMobile.planningAhead.length - 1) {
  ...
  const currentCategory = depressionPlanetMobile.planningAhead[focusedCategoryIndex];
  ...
  return (
    <PlanetLayout
      ...
      primaryColor="#7D6B9D"
      accentColor="#9B8BAD"
      prevRoute="/planets/depression/strengths"
      ...
      <div className="space-y-8 py-6">
        ...
        <div className="flex justify-center items-center gap-4 mb-4">
          ...
          <span className="text-slate-400 text-sm">
            {focusedCategoryIndex + 1} / {depressionPlanetMobile.planningAhead.length}
          </span>
          <button
            onClick={nextCategory}
            disabled={focusedCategoryIndex === depressionPlanetMobile.planningAhead.length - 1}
            className="px-4 py-2 rounded-lg text-sm font-medium transition-colors disabled:opacity-30"
            style={{
              background: "linear-gradient(135deg, rgba(100, 116, 139, 0.15) 0%, rgba(71, 85, 105, 0.1) 100%)",
              border: "1px solid rgba(100, 116, 139, 0.3)",
                  ...
                  <motion.div
                    ...
                    style={{
                      background: expandedCard === card.id
                        ? "linear-gradient(135deg, rgba(125, 107, 157, 0.12) 0%, rgba(125, 107, 157, 0.08) 100%)"
                        : "linear-gradient(135deg, rgba(125, 107, 157, 0.08) 0%, rgba(125, 107, 157, 0.05) 100%)",
                      border: "1px solid rgba(125, 107, 157, 0.2)"
                    }}
        ...
        <div className="text-center pt-8 space-y-4">
          <p className="text-slate-300 text-lg" style={{ lineHeight: "1.6" }}>
            You've explored the Depression Planet
          </p>
          <button
            onClick={() => router.push('/')}
            className="px-6 py-3 rounded-xl font-semibold transition-all duration-200 hover:scale-105"
            style={{
              background: "linear-gradient(135deg, rgba(125, 107, 157, 0.25) 0%, rgba(125, 107, 157, 0.2) 100%)",
              border: "1px solid rgba(125, 107, 157, 0.4)",
              color: "#7D6B9D",
              minHeight: "44px"
            }}
          >
