...
export function ParentingUnderstanding() {
  const router = useRouter();
  const { soundEnabled } = useContext(SoundContext);
  const sections = parentingPlanetMobile.understandingIt.sections;

  const handleNext = () => router.push('/planets/parenting/how-it-shows-up');
  const handlePrev = () => router.push('/planets/parenting/facts');

  return (
    <PlanetLayout currentStep={3} totalSteps={6} nextRoute="/planets/parenting/how-it-shows-up" prevRoute="/planets/parenting/facts" onSwipeLeft={handleNext} onSwipeRight={handlePrev} primaryColor="#FFB6A3" accentColor="#FF847C">
